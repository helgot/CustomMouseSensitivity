...
bool RenderHookDX12::HookGraphicsAPI()
{
    ...
    //    if (MH_CreateHook(pCreateDevice, &Hooked_D3D12CreateDevice,
    //    reinterpret_cast<void**>(&m_original_D3D12CreateDevice)) != MH_OK ||
    ...
    RegisterClassEx(&wc);
    HWND dummy_hwnd =
        CreateWindow(wc.lpszClassName, "DX12 Dummy", WS_OVERLAPPEDWINDOW, 100,
                     100, 300, 300, NULL, NULL, wc.hInstance, NULL);
    ...
    if (FAILED(pDevice->CreateCommandQueue(&queueDesc,
    ...
    if (pFactory)
    {
        ...
        if (SUCCEEDED(
                pFactory->CreateSwapChain(pCommandQueue, &sd, &pTmpSwapChain)))
        {
    ...
    LOG_DEBUG("%s: m_original_ResizeBuffers: 0x%llx", __func__,
              m_original_ResizeBuffers);
    ...
    if (MH_CreateHook(m_target_Present, &Hooked_Present,
                      reinterpret_cast<void **>(&m_original_Present)) != MH_OK)
    {
    ...
    if (MH_CreateHook(m_target_ResizeBuffers, &Hooked_ResizeBuffers,
                      reinterpret_cast<void **>(&m_original_ResizeBuffers)) !=
        MH_OK)
    {
    ...
    if (MH_CreateHook(m_target_ExecuteCommandLists, &Hooked_ExecuteCommandLists,
                      reinterpret_cast<void **>(
                          &m_original_ExecuteCommandLists)) != MH_OK ||
    ...
    LOG_DEBUG("%s: m_original_ResizeBuffers: 0x%llx", __func__,
              m_original_ResizeBuffers);
    LOG_DEBUG("%s: m_original_ExecuteCommandLists: 0x%llx", __func__,
              m_original_ExecuteCommandLists);
...
void RenderHookDX12::InitializeImGui(IDXGISwapChain3 *pSwapChain)
{
    ...
    if (FAILED(m_d3d12Device->CreateDescriptorHeap(
            &rtv_heap_desc, IID_PPV_ARGS(&m_rtvDescriptorHeap))))
    {
    ...
    if (FAILED(m_d3d12Device->CreateDescriptorHeap(
            &srv_heap_desc, IID_PPV_ARGS(&m_srvDescriptorHeap))))
    {
    ...
    if (FAILED(m_d3d12Device->CreateFence(0, D3D12_FENCE_FLAG_NONE,
    ...
    m_original_WndProc = (WNDPROC)SetWindowLongPtrA(
        m_hWnd, GWLP_WNDPROC, (LONG_PTR)RenderHookDX12::Hooked_WndProc);
...
void RenderHookDX12::CreateRenderTarget()
{
    ...
    UINT rtvDescriptorSize = m_d3d12Device->GetDescriptorHandleIncrementSize(
    ...
    for (UINT i = 0; i < m_numBackBuffers; i++)
    {
        ...
        m_d3d12Device->CreateRenderTargetView(m_mainRenderTargetResource[i],
                                              nullptr, rtvHandle);
...
LRESULT CALLBACK RenderHookDX12::Hooked_WndProc(HWND hWnd, UINT uMsg,
...
HRESULT WINAPI RenderHookDX12::Hooked_D3D12CreateDevice(
    ...
    void **ppDevice)
{
    ...
    return pThis->m_original_D3D12CreateDevice(pAdapter, MinimumFeatureLevel,
                                               riid, ppDevice);
}

HRESULT WINAPI RenderHookDX12::Hooked_Present(IDXGISwapChain3 *pSwapChain,
                                              UINT SyncInterval, UINT Flags)
{
    ...
    if (pThis->m_imguiInitialized && isMenuVisible)
    {
        ...
        ID3D12CommandAllocator *commandAllocator =
            pThis->m_commandAllocators[backBufferIdx];
        commandAllocator->Reset();

        ID3D12GraphicsCommandList *commandList = nullptr;
        pThis->m_d3d12Device->CreateCommandList(
        ...
        barrier.Transition.pResource =
            pThis->m_mainRenderTargetResource[backBufferIdx];
        ...
        commandList->OMSetRenderTargets(
            1, &pThis->m_mainRenderTargetDescriptor[backBufferIdx], FALSE,
            nullptr);
        ...
        commandList->Close();
        pThis->m_commandQueue->ExecuteCommandLists(
...
HRESULT WINAPI RenderHookDX12::Hooked_ResizeBuffers(IDXGISwapChain *pSwapChain,
                                                    UINT BufferCount,
                                                    ...
                                                    UINT SwapChainFlags)
{
    ...
    HRESULT hr = pThis->m_original_ResizeBuffers(
...
void WINAPI RenderHookDX12::Hooked_ExecuteCommandLists(
    ...
    ID3D12CommandList *const *ppCommandLists)
{
    ...
    pThis->m_original_ExecuteCommandLists(pQueue, NumCommandLists,
                                          ppCommandLists);
}
