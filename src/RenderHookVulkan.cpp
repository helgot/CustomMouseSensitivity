...
RenderHookVulkan::Hooked_vkGetDeviceProcAddr(VkDevice device, const char *pName)
{
    ...
    if (strcmp(pName, "vkCreateSwapchainKHR") == 0)
    {
        ...
        {
            hook->m_original_vkCreateSwapchainKHR =
                (PFN_vkCreateSwapchainKHR)hook->m_original_vkGetDeviceProcAddr(
                    device, pName);
        }
    ...
    if (strcmp(pName, "vkDestroySwapchainKHR") == 0)
    {
        ...
        {
            hook->m_original_vkDestroySwapchainKHR =
                (PFN_vkDestroySwapchainKHR)hook->m_original_vkGetDeviceProcAddr(
                    device, pName);
        }
    ...
    if (strcmp(pName, "vkQueuePresentKHR") == 0)
    {
        ...
        {
            hook->m_original_vkQueuePresentKHR =
                (PFN_vkQueuePresentKHR)hook->m_original_vkGetDeviceProcAddr(
                    device, pName);
        }
...
bool RenderHookVulkan::HookVulkanFunction(HMODULE vulkanLib,
                                          ...
                                          LPVOID *ppOriginal)
{
    ...
    PFN_vkCreateInstance targetFunction = (PFN_vkCreateInstance)GetProcAddress(
        vulkanLib, targetFunctionName.c_str());
    ...
    if (MH_CreateHook(targetFunction, detourFunction, ppOriginal) != MH_OK)
    {
        ...
        LOG_DEBUG("Address of target function: %p",
                  static_cast<void *>(targetFunction));
        MH_Uninitialize();
        return false;
    }
...
bool RenderHookVulkan::HookGraphicsAPI()
{
    ...
    if (!HookVulkanFunction(vulkanLib, "vkCreateInstance",
                            RenderHookVulkan::Hooked_vkCreateInstance,
                            (LPVOID *)&m_original_vkCreateInstance))
        return false;
    if (!HookVulkanFunction(vulkanLib, "vkCreateDevice",
                            RenderHookVulkan::Hooked_vkCreateDevice,
                            (LPVOID *)&m_original_vkCreateDevice))
        return false;
    if (!HookVulkanFunction(vulkanLib, "vkGetDeviceProcAddr",
                            RenderHookVulkan::Hooked_vkGetDeviceProcAddr,
                            (LPVOID *)&m_original_vkGetDeviceProcAddr))
        return false;
...
VkResult RenderHookVulkan::CreateRenderPass()
{
    ...
    LOG_DEBUG("%s: g_vkDevice at entry: %p", __func__,
              (void *)m_vkDevice); // LOG_DEBUG g_vkDevice value
    ...
    LOG_DEBUG("%s: Attempting to create render pass with VkDevice: %p",
              __func__, (void *)m_vkDevice);
    return vkCreateRenderPass(m_vkDevice, &renderPassInfo, nullptr,
                              &m_imguiRenderPass);
}
...
VkResult RenderHookVulkan::CreateDescriptorPool()
{
    ...
    return vkCreateDescriptorPool(m_vkDevice, &pool_info, nullptr,
...
void RenderHookVulkan::InitializeImGui()
{
    ...
    if (m_vkInstance == VK_NULL_HANDLE || m_vkDevice == VK_NULL_HANDLE ||
        m_vkPhysicalDevice == VK_NULL_HANDLE || m_vkQueue == VK_NULL_HANDLE ||
        m_swapchainImages.empty() || m_imguiRenderPass == VK_NULL_HANDLE)
    {
        LOG_DEBUG(
            ...
            __func__, (void *)m_vkInstance, (void *)m_vkPhysicalDevice,
            (void *)m_vkDevice, (void *)m_vkQueue, m_swapchainImages.size(),
            (void *)m_imguiRenderPass);
        return;
    }
    ...
    io.ConfigFlags |=
        ImGuiConfigFlags_NoMouseCursorChange; // Don't change mouse cursor

    // 3. Setup Dear ImGui style
    ImGui::StyleColorsDark();

    // 4. Setup Platform backends
    { // PLATFORM SPECIFIC CODE... Could replace with a function pointer or
      // similar to avoid platform-specific code here.
        // 1. Get window handle
        ...
        else
        {
            ...
            m_original_WndProc = (WNDPROC)SetWindowLongPtrA(
                m_hWnd, GWLP_WNDPROC,
                (LONG_PTR)RenderHookVulkan::Hooked_WndProc);
    ...
    // Create ImGui descriptor pool if not already created (e.g., in
    ...
    init_info.Instance = m_vkInstance; // g_vkInstance needs to be captured
                                       // (e.g., from vkCreateInstance hook)
    ...
    init_info.MinImageCount =
        (uint32_t)m_swapchainImages.size(); // Use actual swapchain image count
    ...
    ImGui_ImplVulkan_Init(
        &init_info); // Removed the direct RenderPass parameter

    // Font texture upload is now handled automatically by
    // ImGui_ImplVulkan_NewFrame() The previous manual font upload code has been
    // removed as per ImGui changeLOG_DEBUG.
...
LRESULT CALLBACK RenderHookVulkan::Hooked_WndProc(HWND hWnd, UINT uMsg,
                                                  WPARAM wParam, LPARAM lParam)
{
    auto pThis = static_cast<RenderHookVulkan *>(s_instance);
    // For custom application messages, pass them directly to the game's
    // original handler to prevent ImGui from interfering with sensitive,
    // proprietary message loops.
    if (uMsg >= WM_APP)
    {
        if (pThis && pThis->m_original_WndProc)
        {
            return CallWindowProc(pThis->m_original_WndProc, hWnd, uMsg, wParam,
                                  lParam);
        }
    }
...
VkResult VKAPI_PTR RenderHookVulkan::Hooked_vkQueuePresentKHR(
    VkQueue queue, const VkPresentInfoKHR *pPresentInfo)
{
    ...
    if (pThis->m_guiCallback)
    {
        ...
        // 这里假设 m_guiCallback 是 UIManager::Render，且 UIManager 内部有
        // m_isMenuVisible 推荐将 UIManager::m_isMenuVisible 暴露为 public
        // 或加一个 getter 这里用静态变量 hack（实际建议用 getter）
        ...
        isMenuVisible)
    {
        ...
        VkSemaphore currentFrameSemaphore =
            pThis->m_renderCompleteSemaphores[imageIndex];

        vkResetCommandPool(pThis->m_vkDevice,
                           pThis->m_imguiCommandPools[imageIndex], 0);
        ...
        if (vkAllocateCommandBuffers(pThis->m_vkDevice, &allocInfo,
                                     &imguiCmdBuffer) != VK_SUCCESS)
        {
        ...
        renderPassBeginInfo.framebuffer =
            pThis->m_imguiFramebuffers[imageIndex];
        renderPassBeginInfo.renderArea.extent = pThis->m_swapchainExtent;

        vkCmdBeginRenderPass(imguiCmdBuffer, &renderPassBeginInfo,
        ...
        vkWaitForFences(pThis->m_vkDevice, 1, &currentFrameFence, VK_TRUE,
                        UINT64_MAX);

        vkFreeCommandBuffers(pThis->m_vkDevice,
                             pThis->m_imguiCommandPools[imageIndex], 1,
                             &imguiCmdBuffer);
...
VkResult VKAPI_PTR RenderHookVulkan::Hooked_vkCreateDevice(
    ...
    const VkAllocationCallbacks *pAllocator, VkDevice *pDevice)
{
    ...
    VkResult result = pThis->m_original_vkCreateDevice(
    ...
    if (result == VK_SUCCESS)
    {
        ...
        LOG_DEBUG("%s: Captured VkDevice: %p, VkPhysicalDevice: %p", __func__,
                  (void *)pThis->m_vkDevice, (void *)pThis->m_vkPhysicalDevice);
        ...
        vkGetDeviceQueue(pThis->m_vkDevice, queueFamilyIndex, 0,
                         &pThis->m_vkQueue);
        LOG_DEBUG("%s: Captured VkQueue: %p", __func__,
                  (void *)pThis->m_vkQueue);
    }
    return result;
}
void VKAPI_PTR RenderHookVulkan::Hooked_vkDestroySwapchainKHR(
    VkDevice device, VkSwapchainKHR pSwapchain,
...
VkResult VKAPI_PTR RenderHookVulkan::Hooked_vkCreateSwapchainKHR(
    VkDevice device, const VkSwapchainCreateInfoKHR *pCreateInfo,
    const VkAllocationCallbacks *pAllocator, VkSwapchainKHR *pSwapchain)
{
    LOG_DEBUG("%s: Called.", __func__);
    RenderHookVulkan *pThis = static_cast<RenderHookVulkan *>(s_instance);
    VkResult result = pThis->m_original_vkCreateSwapchainKHR(
        device, pCreateInfo, pAllocator, pSwapchain);

    if (result == VK_SUCCESS)
    {
        pThis->m_vkSwapchain =
            *pSwapchain; // Store the created swapchain object
        ...
        LOG_DEBUG("%s: Swapchain Format: %d, Extent: %dx%d", __func__,
                  pThis->m_swapchainImageFormat, pThis->m_swapchainExtent.width,
                  pThis->m_swapchainExtent.height);
        ...
        for (uint32_t i = 0; i < imageCount; ++i)
        {
            if (vkCreateSemaphore(device, &semaphoreInfo, nullptr,
                                  &pThis->m_renderCompleteSemaphores[i]) !=
                    VK_SUCCESS ||
                vkCreateFence(device, &fenceInfo, nullptr,
                              &pThis->m_renderFences[i]) != VK_SUCCESS)
            {
                LOG_DEBUG(
        ...
        if (pThis->m_vkDevice == VK_NULL_HANDLE)
        { // Double-check g_vkDevice here
            LOG_DEBUG("%s: CRITICAL ERROR: g_vkDevice is NULL before calling "
                      ...
                      __func__);
            // This indicates a severe issue in the Vulkan initialization flow
            // or our hook order. We cannot proceed with ImGui initialization if
            // the device is not available.
            return result; // Return the original result, but ImGui won't
                           // initialize.
        }
        ...
        for (uint32_t i = 0; i < imageCount; ++i)
        {
            ...
            if (vkCreateCommandPool(device, &cmdPoolInfo, nullptr,
                                    &pThis->m_imguiCommandPools[i]) !=
                VK_SUCCESS)
            {
                LOG_DEBUG("%s: Error: Failed to create command pool for ImGui "
                          "command buffer %u!",
                          __func__, i);
                // Handle error
            }
        }
        LOG_DEBUG("%s: Created image views, framebuffers, and command pools "
                  "for ImGui.",
                  __func__);
...
VkResult VKAPI_PTR RenderHookVulkan::Hooked_vkCreateInstance(
    ...
    const VkAllocationCallbacks *pAllocator, VkInstance *pInstance)
{
    ...
    VkResult result = pThis->m_original_vkCreateInstance(&modifiedCreateInfo,
                                                         pAllocator, pInstance);

    if (result == VK_SUCCESS)
    {
        pThis->m_vkInstance = *pInstance;
        LOG_DEBUG("%s: Captured VkInstance: %p", __func__,
                  (void *)pThis->m_vkInstance);
    }
