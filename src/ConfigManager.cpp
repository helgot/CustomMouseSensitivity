...
{
    j = {{"first_person_sensitivity", config.first_person_sensitivity},
...
{
    // Using .value() is safer than .at() as it provides a default if the key is
    // missing
...
void ConfigManager::LoadConfig()
{
    ...
    if (!file.is_open())
    {
        LOG_INFO("%s not found. Creating default configuration.",
                 kConfigFileName.data());
    ...
    {
        LOG_ERROR("Failed to parse %s: %s. Loading default config.",
                  kConfigFileName.data(), e.what());
