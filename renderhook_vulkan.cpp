...
void RenderHookVulkan::Shutdown()
{
	...
	// Probably shouldn't destroy the device here, as the hooked application manages the Vulkan device.
...
void RenderHookVulkan::DeinitializeImGui()
{
    ...
    // Destroy framebuffers and image views created by the hook
    ...
    // Destroy swapchain object itself if we created it (unlikely in a hook)
    // If the game manages it, we don't destroy it.
    // However, if we hooked vkDestroySwapchainKHR, we'd handle it there.
    // For now, assuming game handles its own swapchain destruction.
...
LRESULT CALLBACK RenderHookVulkan::Hooked_WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (uMsg >= WM_APP) {
        LOG_DEBUG("Hooked_WndProc: Passing custom message %u directly to original WndProc.", uMsg);
...
VkResult VKAPI_PTR RenderHookVulkan::Hooked_vkCreateSwapchainKHR(VkDevice device, const VkSwapchainCreateInfoKHR* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkSwapchainKHR* pSwapchain)
{
    ...
    if (result == VK_SUCCESS) {
        ...
        if (pThis->m_imguiRenderPass == VK_NULL_HANDLE) {
...
PFN_vkVoidFunction VKAPI_PTR RenderHookVulkan::Hooked_vkGetDeviceProcAddr(VkDevice device, const char* pName)
{
    ...
    PFN_vkVoidFunction originalFunc = pThis->m_original_vkGetDeviceProcAddr(device, pName);
    LOG_DEBUG("vkGetDeviceProcAddr requested: %s", pName);

    // device-level functions;
    if (strcmp(pName, "vkCreateSwapchainKHR") == 0) {
        LOG_DEBUG("Intercepting vkCreateSwapchainKHR!");
        pThis->m_original_vkCreateSwapchainKHR = (PFN_vkCreateSwapchainKHR)originalFunc;
        return (PFN_vkVoidFunction)RenderHookVulkan::Hooked_vkCreateSwapchainKHR;
    }
    else if (strcmp(pName, "vkQueuePresentKHR") == 0) {
        LOG_DEBUG("Intercepting vkQueuePresentKHR!");
        pThis->m_original_vkQueuePresentKHR = (PFN_vkQueuePresentKHR)originalFunc;
        return (PFN_vkVoidFunction)RenderHookVulkan::Hooked_vkQueuePresentKHR;
    }
    return originalFunc;
}
